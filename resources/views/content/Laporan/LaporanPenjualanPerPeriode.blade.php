...
@section('content')
    ...
    <h3 class="page-title">
        <b>Laporan Penjualan per Periode</b> <small>Mengelola Penjualan per Periode</small>
    </h3>
    <br/>
    <div id="accordion">
        <form method="post" action="{{route('filter-laporan-penjualan-per-periode')}}" enctype="multipart/form-data">
            @csrf
    ...
    <div class="card border border-dark">
        <div class="card-header bg-dark clearfix">
            ...
            <div class="form-actions float-right">
                <button onclick="location.href='{{ url('/laporan-penjualan-per-periode/cetak-pdf') }}'" class="btn btn-sm btn-info"
                    title="Add Data" style="margin-left: 5px">Print PDF
                </button>
                <button onclick="location.href='{{ url('/laporan-penjualan-per-periode/cetak-excel') }}'" class="btn btn-sm btn-info"
                    ...
                    <tbody>
                        ...
                        @foreach ($sales_item as $si)
                            <tr>
                                ...
                                <td width="40%" >{{ $si->callProduct['product_name'] }}</td>
                                <td width="20%" style='text-align:center'>{{$si['sales_item_amount']}}</td>
