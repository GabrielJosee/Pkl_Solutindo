...
class LaporanPembelianPerProdukController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        return view('content/Laporan/LaporanPembelianPerProduk', compact('purchase', 'start_date', 'end_date', 'product', 'product_id'));
    }
    ...
    public function filter(Request $request)
    {
        ...
        return redirect('/laporan-pembelian-per-produk');
    }

    public function CetakPDF()
    {
        $filename = 'Laporan Pembelian per Produk.pdf';

        $purchase = Purchase::where('data_state', 0)->get();
        $total_price = Purchase::where('data_state', 0)->get();
        ...
        $purchase = $purchase->where('created_at', '>=', $start_date)->where('created_at', '<=', $stop_date);
        $total_price = $total_price->where('created_at', '>=', $start_date)->where('created_at', '<=', $stop_date)->sum('purchase_price');
        
        $html = view()->make('content/Laporan/CetakLaporanPembelianPerProduk', ['purchase' => $purchase, 'start_date' => $start_date, 'end_date' => $end_date, 'total_price' => $total_price])->render();
        $pdf = new PDF;
        $pdf::SetTitle('Laporan Pembelian per Produk');
        $pdf::AddPage();
        $pdf::writeHTML($html);
    ...
    public function CetakExcel()
    {
        ...
        $purchase = $purchase->where('created_at', '>=', $start_date)->where('created_at', '<=', $stop_date);
        $total_price = $total_price->where('data_state', 0)->where('created_at', '>=', $start_date)->where('created_at', '<=', $stop_date)->sum('purchase_price');
    ...
    $activeWorksheet->setCellValue('A1', 'Laporan Pembelian Per Produk');
    ...
    $writer->save('Laporan Pembelian per Produk.xlsx');
    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    header('Content-Disposition: attachment;filename="Laporan Pembelian per Produk.xlsx"');
